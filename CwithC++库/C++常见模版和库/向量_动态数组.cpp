...
C++的std::vector容器本身并没有find方法，
但是你可以使用标准库中的std::find函数来查找vector中的元素。
这个函数需要一个范围（通过两个迭代器定义）和一个值，
然后返回一个指向找到的第一个元素的迭代器。
如果在范围内没有找到该元素，std::find函数将返回end()迭代器

C++的std::vector容器本身并没有count方法，
但是你可以使用标准库中的std::count函数来统计vector中特定元素的个数。
这个函数需要一个范围（通过两个迭代器定义）和一个值，
然后返回这个值在范围内出现的次数
